...
export function NavBar({ items, className }: NavBarProps) {
  ...
  return (
    <div
      className={cn(
        "fixed bottom-0 sm:bottom-auto sm:top-0 left-1/2 -translate-x-1/2 z-50 mb-6 sm:mb-0 sm:pt-6 pointer-events-none pb-safe",
        className,
      )}
    >
      <div className="flex items-center gap-3 bg-white/90 backdrop-blur-lg py-1 px-1 rounded-full shadow-lg border border-gray-200/50 pointer-events-auto">
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <motion.div
                  layoutId="lamp"
                  className="absolute inset-0 w-full bg-purple-100/50 rounded-full -z-10"
                  initial={false}
                  transition={{
                    type: "spring",
                    stiffness: 300,
