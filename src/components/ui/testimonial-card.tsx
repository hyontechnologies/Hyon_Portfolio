...
const StickyTestimonialCard = ({ testimonial, index }: { testimonial: Testimonial; index: number }) => {
  return (
    <motion.div
      className="relative lg:sticky w-full"
      style={{ top: `calc(120px + ${index * 24}px)` }}
    >
      <figure className={cn(
        "p-6 rounded-2xl flex flex-col h-auto w-full transition-all duration-300 m-0",
        ...
        <figcaption className="flex items-center gap-4">
          <div className="flex-grow">
            <cite className="font-semibold text-lg text-white not-italic block">{testimonial.name}</cite>
        ...
        {/* Quote */}
        {testimonial.quote && (
          <blockquote className="text-base text-gray-300 leading-relaxed">
...
export const ClientsSection = ({
  ...
  return (
    <section className={cn("w-full bg-transparent text-white py-16", className)}>
