...
export const Radar = ({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[2px] w-full bg-gradient-to-r from-transparent via-purple-400 to-transparent" />
      </div>
      {/* Concentric full circles */}
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(168, 85, 247, ${Math.max(0.05, 0.25 - idx * 0.025)})`,
            boxShadow: `0 0 15px rgba(168, 85, 247, ${Math.max(0.0, 0.15 - idx * 0.02)}) inset, 0 0 15px rgba(168, 85, 247, ${Math.max(0.0, 0.15 - idx * 0.02)})`
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
